...
function PhaseBadge({ phase, timer }: { phase: Phase; timer?: number | null }) {
  const map: Record<Phase, string> = {
    waiting: "Waiting",
    memoryReveal: "Memory",
    firstBetting: "Betting",
    draw: "Draw",
    drawReveal: "Reveal",
    secondBetting: "Betting",
    showdown: "Showdown",
  };
  const isEye = phase === "memoryReveal" || phase === "drawReveal";
  return (
    <div className="flex items-center gap-1 px-2 py-1 rounded-full bg-black/60 gold-border backdrop-blur-sm">
      {isEye
        ? <Eye className="w-2.5 h-2.5 text-[color:var(--color-gold)]" />
        : <EyeOff className="w-2.5 h-2.5 text-[color:var(--color-gold)]" />
      }
      <span className="font-display text-[8px] font-semibold tracking-widest uppercase gold-text whitespace-nowrap">
        {map[phase]}
      </span>
      {typeof timer === "number" && (
        <span className="flex items-center gap-0.5 text-[8px] font-bold text-[color:var(--color-gold)]">
...
function PlayerSeat({
  name, chips, bet, active, avatar, folded,
}: {
  name: string; chips: number; bet?: number; active?: boolean; avatar: string; folded?: boolean;
}) {
  return (
    <div className="relative shrink-0">
      ...
      <div className={cn(
        ...
        <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-full grid place-items-center font-display font-bold text-[9px] sm:text-sm shrink-0 bg-gradient-to-br from-[color:var(--color-gold)] to-[color:var(--color-gold-soft)] text-[color:var(--color-felt-deep)]">
          {avatar}
        </div>
...
// ─── Decorative fanned cards for the main menu ────────────────────────────────
...
export default function App() {
      ...
      <div className="h-dvh bg-[var(--color-background)] flex flex-col landscape:flex-row overflow-hidden">
        ...
        <div className="flex-1 relative flex flex-col items-center justify-center gap-4 p-6 min-h-0">
          ...
          <div className="relative text-center">
            <h1 className="font-display text-3xl landscape:text-2xl font-bold gold-text leading-tight">
              PokerUrMemory
            </h1>
        ...
        <div className="shrink-0 flex flex-col justify-center gap-3 p-5 landscape:p-6 landscape:w-72 landscape:border-l landscape:border-white/[0.07] bg-black/30 overflow-y-auto">
          <p className="font-display text-[9px] tracking-[0.25em] text-[color:var(--color-gold)]/60 uppercase landscape:mt-0 -mb-1">
            Your Name
          </p>
          <input
            type="text"
        ...
        <div className="flex-1 flex flex-col p-5 landscape:p-6 min-h-0 overflow-y-auto">
          ...
          <div className="flex items-center gap-3 mb-5">
            <div className="flex-1 min-w-0">
              ...
              <p className="font-display text-3xl landscape:text-2xl font-bold gold-text tracking-widest leading-none">
                {roomCode}
              </p>
            </div>
            <button
          ...
          {/* Player list */}
          <p className="text-[8px] text-gray-600 uppercase tracking-[0.2em] mb-2">
          ...
          {isHost ? (
            ...
            <div className="flex items-center gap-2">
              {[0, 1, 2].map(i => (
                <div
                  key={i}
                  className="w-1.5 h-1.5 rounded-full bg-[color:var(--color-gold)]/50 animate-pulse"
  ...
  // Chip variant based on pot size
  ...
  return (
    <main className="h-dvh w-full bg-[radial-gradient(ellipse_at_top,oklch(0.22_0.04_150)_0%,oklch(0.10_0.02_150)_100%)] text-foreground overflow-hidden relative">
      ...
      {showExitDialog && (
        <ConfirmDialog
          title="Leave Game?"
          message="You'll be disconnected from the current table. Any chips in the pot will be forfeited."
          confirmLabel="Leave"
          cancelLabel="Stay"
      ...
      {(isMemoryReveal || isDrawReveal) && typeof timer === "number" && (
        <div className="fixed inset-0 z-[200] pointer-events-none flex items-center justify-center bg-black/40">
          <div className="flex flex-col items-center">
            <span className="font-display text-[9px] tracking-[0.3em] uppercase text-[color:var(--color-gold)] mb-1">
              {isMemoryReveal ? "Memorize!" : "Draw Reveal!"}
            </span>
            <span
              className="font-display font-black leading-none text-[72px] sm:text-[110px] animate-pulse"
              style={{ color: "var(--color-gold)", textShadow: "0 0 40px rgba(212,168,67,0.8)" }}
            >
              {timer}
            </span>
          </div>
                ...
                {showdownData.hands.map((h: any, i: number) => {
                  ...
                  return (
                    <div key={i} className={cn(
                      "flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-[10px] sm:text-xs",
                      isWinner
                        ? "bg-[color:var(--color-gold)]/20 border border-[color:var(--color-gold)]/50"
                        : "bg-white/5",
                    )}>
      ...
      {/* ── FIXED TOP BAR ────────────────────────────────────────────────────── */}
      <div className="fixed top-2.5 left-2.5 right-2.5 z-30 flex items-center justify-between pointer-events-none">
      ...
      <div className="absolute top-[14%] left-0 right-0 flex justify-center items-start gap-3 sm:gap-6 px-2 z-10">
        {opponents.map((opp, idx) => (
          <div key={idx} className={cn(
            "flex flex-col items-center gap-1",
            opp.folded && "opacity-30 grayscale",
          )}>
            <PlayerSeat
              name={opp.name}
              chips={opp.chips}
      ...
      {/* ── CENTER — pot (ChipStack) & action log ────────────────────────────── */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 flex flex-col items-center gap-1.5">
        <div className="px-4 pt-7 pb-2.5 rounded-xl bg-black/50 gold-border backdrop-blur-md shadow-xl">
          <ChipStack amount={Math.max(1, gameState.pot)} variant={potChipVariant} />
        </div>
      ...
      {/* ── BET SLIDER PANEL — slides in from right ───────────────────────────── */}
      <div className={cn(
        ...
        <div className="pt-5 mb-3">
          <ChipStack amount={raiseAmount[0]} variant="red" />
        </div>
