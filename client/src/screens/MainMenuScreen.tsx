    ...
    >
      ...
      <div className="relative z-10 shrink-0 flex items-center justify-between gap-3
        ...
        <div className="flex gap-1.5 sm:gap-2.5 lg:gap-3 shrink-0">
          {stats.map(s => (
            <div key={s.label} className="flex flex-col items-center bg-white rounded-xl lg:rounded-2xl px-2.5 py-1.5 sm:px-4 sm:py-2 lg:px-5 lg:py-3 border border-black/[0.07] shadow-md min-w-[46px] sm:min-w-[58px] lg:min-w-[80px]">
              <span className="font-display text-sm sm:text-lg lg:text-2xl font-bold blue-text leading-tight">{s.value}</span>
              <span className="text-[8px] sm:text-[9px] lg:text-[10px] text-gray-300 tracking-wide lg:tracking-widest uppercase mt-0.5">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 min-h-0 flex flex-col items-center justify-center gap-6 sm:gap-8 lg:gap-10 px-6">
        <div className="text-center">
          <h1 className="font-display text-2xl min-[380px]:text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-bold leading-tight tracking-wide text-white drop-shadow-md whitespace-nowrap">
            ♠ PokerUrMemory ♠
          </h1>
        ...
        <div className="w-full max-w-[300px] sm:max-w-[340px] lg:w-[380px] lg:max-w-none flex flex-col gap-3 lg:gap-4">
          ...
          <button
            ...
            className="w-full h-12 lg:h-14 rounded-2xl font-display tracking-wider lg:tracking-[0.15em] uppercase text-[11px] lg:text-[12px] font-bold bg-white text-[color:var(--color-blue)] border border-[color:var(--color-blue)]/30 shadow-md hover:bg-white/90 active:scale-[0.97] transition-all flex items-center justify-center gap-2 lg:gap-2.5"
          >
      ...
      <div className="relative z-10 shrink-0 flex flex-wrap items-end justify-start sm:justify-between gap-2
        ...
        <div className="flex gap-2">
          <button
            onClick={onProfile}
            className="h-11 sm:h-12 px-4 sm:px-5 lg:px-6 rounded-2xl font-display tracking-wider uppercase text-[10px] sm:text-[11px] font-bold bg-white text-foreground border border-black/[0.10] shadow-md hover:bg-white/90 active:scale-[0.97] transition-all flex items-center justify-center gap-2"
          >
