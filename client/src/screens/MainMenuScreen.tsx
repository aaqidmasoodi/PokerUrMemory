...
export function MainMenuScreen({
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 shrink-0 flex items-center justify-between gap-3
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-bold text-xs sm:text-base lg:text-xl text-white leading-tight truncate landscape:text-[10px] landscape:sm:text-xs">
              {profile.username}
              {profile.country_code && (
                <span className="ml-1.5">{getFlagEmoji(profile.country_code)}</span>
              )}
            </p>
            <p className="text-[10px] sm:text-xs text-gray-300 mt-0.5 truncate landscape:text-[8px] landscape:sm:text-[9px]">
        ...
        <div className="flex gap-1.5 sm:gap-2.5 lg:gap-3 shrink-0">
          {stats.map(s => (
            <div key={s.label} className="flex flex-col items-center bg-white rounded-xl lg:rounded-2xl px-2 py-1 sm:px-4 sm:py-2 lg:px-5 lg:py-3 border border-black/[0.07] shadow-md min-w-[40px] sm:min-w-[58px] lg:min-w-[80px]
              ...
              <span className="font-display text-xs sm:text-lg lg:text-2xl font-bold blue-text leading-tight landscape:text-[9px] landscape:sm:text-xs">{s.value}</span>
              <span className="text-[7px] sm:text-[9px] lg:text-[10px] text-gray-300 tracking-wide lg:tracking-widest uppercase mt-0.5 landscape:text-[6px] landscape:sm:text-[7px]">{s.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="relative z-10 flex-1 min-h-0 flex flex-col items-center justify-center gap-6 sm:gap-8 lg:gap-10 px-6 landscape:gap-2 landscape:sm:gap-3">
        <div className="text-center">
          ...
          <p className="text-[10px] sm:text-xs lg:text-sm text-gray-300 mt-2 lg:mt-3 tracking-[0.2em] lg:tracking-[0.28em] uppercase landscape:text-[8px] landscape:sm:text-[9px] landscape:mt-1">
      ...
      <div className="relative z-10 shrink-0 flex flex-wrap items-end justify-start sm:justify-between gap-2
        ...
        <div className="flex gap-2">
          ...
          >
            <User className="w-3 h-3 lg:w-4 lg:h-4 text-[color:var(--color-blue)] landscape:w-2.5" />
            Profile
          </button>
          <button
            onClick={onSettings}
        ...
        <button
          ...
          className="h-10 sm:h-12 px-3 sm:px-5 lg:px-6 rounded-2xl font-display tracking-wider uppercase text-[9px] sm:text-[11px] font-bold bg-white text-foreground border border-black/[0.10] shadow-md hover:bg-white/90 active:scale-[0.97] transition-all flex items-center justify-center gap-2
            landscape:h-9 landscape:sm:h-10 landscape:text-[8px] landscape:sm:text-[9px] landscape:px-2"
        >
          <BookOpen className="w-3 h-3 lg:w-4 lg:h-4 text-[color:var(--color-blue)] landscape:w-2.5" />
          How to Play
        </button>
      </div>
    </div>
